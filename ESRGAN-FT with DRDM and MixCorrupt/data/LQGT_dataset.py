...
class LQGTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # generate NF image
        if self.opt['target']:
            img_NF = util.read_img(None, self.paths_NF[index])
        else:
            img_NF = util.modcrop(img_GT, scale)
        # change color space if necessary
        if self.opt['color']:
            img_NF = util.channel_convert(img_NF.shape[2], self.opt['color'], [img_GT])[0]
        ...
        if True: #self.opt['phase'] == 'train':
            ...
            rnd_nf_h = random.randint(0, max(0, H_ - LQ_size))
            rnd_nf_w = random.randint(0, max(0, W_ - LQ_size))
            img_LQ = img_LQ[rnd_h:rnd_h + LQ_size, rnd_w:rnd_w + LQ_size, :]
            img_NF = img_NF[rnd_nf_h:rnd_nf_h + LQ_size, rnd_nf_w:rnd_nf_w + LQ_size, :]
        ...
        if LQ_path is None:
            LQ_path = GT_path
        return {'LQ': img_LQ, 'GT': img_GT, 'NF': img_NF, 'LQ_path': LQ_path, 'GT_path': GT_path}
