...
class MRRDBNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4):
        '''only for scale = 2,3,4,8 or 1(No upsampling)'''
        super(RRDBNet, self).__init__()
        RRDB_block_f = functools.partial(RRDB, nf=nf, gc=gc)
        ...
        if (scale and scale - 1) is not True:
            print(scale)
            ...
            raise NotImplementedError("Please choose right scale.")
        self.HRconv = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.conv_last = nn.Conv2d(nf, out_nc, 3, 1, 1, bias=True)
    ...
    def forward(self, x):
        ...
        if self.scale > 1:
            ...
            if self.scale == 8:
                fea = self.lrelu(self.upsampler(self.upconv2(fea)))
                fea = self.lrelu(self.upsampler(self.upconv3(fea)))
        out = self.conv_last(self.lrelu(self.HRconv(fea)))
