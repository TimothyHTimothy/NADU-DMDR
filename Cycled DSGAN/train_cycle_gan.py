...
parser.add_argument('--device_ids', default=(0,1), type=tuple, help='number of workers used')
parser.add_argument('--num_workers', default=4, type=int, help='number of workers used')
parser.add_argument('--num_epochs', default=60, type=int, help='total train epoch number')
parser.add_argument('--num_decay_epochs', default=240, type=int, help='number of epochs during which lr is decayed')
...
parser.add_argument('--num_blocks', default=4, type=int, help='number of Residual CA blocks')
parser.add_argument('--num_groups', default=2, type=int, help='number of RCA Groups')
...
parser.add_argument('--w_tex', default=0.01, type=float, help='weight of texture loss')
parser.add_argument('--w_per', default=0.02, type=float, help='weight of perceptual loss')
parser.add_argument('--w_perc', default=0.02, type=float, help='weight of perceptual weight in cycle consistency loss')
parser.add_argument('--w_cyc', default=0.0001, type=float, help='weight of cycle consistency loss')
parser.add_argument('--checkpoint_noisy2clean', default=None, type=str, help='checkpoint model A to start from')
parser.add_argument('--checkpoint_clean2noisy', default=None, type=str, help='checkpoint model B to start from')
...
if opt.dataset == 'aim2019':
    ...
    if opt.sr_type == 'SDSR':
        train_set = loader.TrainDataset(PATHS[opt.dataset][opt.artifacts]['hr']['train'],
                                    cropped=True, **vars(opt))
    elif opt.sr_type == 'TDSR':
        train_set = loader.TrainDataset(PATHS[opt.dataset][opt.artifacts]['hr']['train'],
                                    target_dir=PATHS[opt.dataset]['clean']['hr']['train'],
                                    cropped=True, **vars(opt))
    train_loader = DataLoader(dataset=train_set, num_workers=opt.num_workers, batch_size=opt.batch_size, shuffle=True)
...
model_g_a = model.Generator(n_blocks=opt.num_blocks, n_groups=opt.num_groups)
model_g_b = model.Generator(n_blocks=opt.num_blocks, n_groups=opt.num_groups)
...
if torch.cuda.is_available():
    ...
    #model_g_a = nn.DataParallel(model_g_a, device_ids=opt.device_ids)
    model_g_b = model_g_b.cuda()
    #model_g_b = nn.DataParallel(model_g_a, device_ids=opt.device_ids)
    model_d_a = model_d_a.cuda()
    #model_d_a = nn.DataParallel(model_g_a, device_ids=opt.device_ids)
    model_d_b = model_d_b.cuda()
    #model_d_b = nn.DataParallel(model_g_a, device_ids=opt.device_ids)
...
if opt.checkpoint_clean2noisy is not None and opt.checkpoint_noisy2clean is not None:
    checkpoint_A = torch.load(opt.checkpoint_clean2noisy)
    checkpoint_B = torch.load(opt.checkpoint_noisy2clean)
    start_epoch = 1 # checkpoint_A['epoch'] + 1
    iteration = checkpoint_A['iteration'] + 1
    new_state_dict = OrderedDict()
    for k, v in checkpoint_A['model_g_state_dict'].items():
        name = k[7:]
        new_state_dict[name] = v
    model_g_a.load_state_dict(new_state_dict)
    new_state_dict = OrderedDict()
    for k, v in checkpoint_A['models_d_state_dict'].items():
        name = k[7:]
        new_state_dict[name] = v 
    model_d_a.load_state_dict(new_state_dict)
    new_state_dict = OrderedDict()
    for k, v in checkpoint_B['model_g_state_dict'].items():
        name = k[7:]
        new_state_dict[name] = v
    model_g_b.load_state_dict(new_state_dict)
    new_state_dict = OrderedDict()
    for k, v in checkpoint_B['models_d_state_dict'].items():
        name = k[7:]
        new_state_dict[name] = v
    model_d_b.load_state_dict(new_state_dict)
    #optimizer_g.load_state_dict(checkpoint['optimizer_g_state_dict'])
    #optimizer_d.load_state_dict(checkpoint['optimizer_d_state_dict'])
    scheduler_g.load_state_dict(checkpoint_A['scheduler_g_state_dict'])
    scheduler_d.load_state_dict(checkpoint_A['scheduler_d_state_dict'])
...
for epoch in range(start_epoch, opt.num_epochs + 1):
    ...
    for target_img, source_img in train_bar:
        ...
        if opt.ragan:
            real_source_tex = model_d_a(source_img, fake_source_img.detach())
            fake_source_tex = model_d_a(fake_source_img.detach(), source_img)
            real_target_tex = model_d_b(target_img, fake_target_img.detach())
            fake_target_tex = model_d_b(fake_target_img.detach(), target_img)
        else:
            real_source_tex = model_d_a(source_img)
            fake_source_tex = model_d_a(fake_source_img.detach())
            real_target_tex = model_d_b(target_img)
            fake_target_tex = model_d_b(fake_target_img.detach())
        ...
        if iteration % opt.gen_freq == 0:
            ...
            model_g_a.zero_grad()
            model_g_b.zero_grad()
            g_loss_a = g_loss_module(fake_source_tex, fake_source_img, target_img)
            assert not torch.isnan(g_loss_a), 'Generator A loss returns NaN values'
            g_loss_b = g_loss_module(fake_target_tex, fake_target_img, source_img)
            assert not torch.isnan(g_loss_b), 'Generator B loss returns NaN values'
            if opt.w_cyc > 0:
                recovered_source_img = model_g_a(fake_target_img)
                cycle_loss_a = opt.w_perc * cycle_per_loss_module(recovered_source_img, source_img) + cycle_dis_loss_module(recovered_source_img, source_img)
                recovered_target_img = model_g_b(fake_source_img)
                cycle_loss_b = opt.w_perc * cycle_per_loss_module(recovered_target_img, target_img) + cycle_dis_loss_module(recovered_target_img, target_img) 
            else:
                cycle_loss_a = 0
                cycle_loss_b = 0
        ...
        # save data to tensorboard
        rgb_loss_a = g_loss_module.rgb_loss(fake_source_img, target_img)
        rgb_loss_b = g_loss_module.rgb_loss(fake_target_img, source_img)
        rgb_loss = rgb_loss_a + rgb_loss_b
        mean_loss_a = g_loss_module.mean_loss(fake_source_img, target_img)
        mean_loss_b = g_loss_module.mean_loss(fake_target_img, source_img)
    ...
    if epoch % opt.val_interval == 0 or epoch % opt.val_img_interval == 0:
        ...
        with torch.no_grad():
            ...
            mse_AB_sum = psnr_AB_sum = 0
            per_loss_A_sum = per_loss_B_sum = 0
            mse_BA_sum = psnr_BA_sum = 0

            # validate on each image in the val dataset
            for index, (source_img, target_img) in enumerate(val_bar):
                if torch.cuda.is_available():
                    source_img = source_img.cuda()
                    target_img = target_img.cuda()
                fake_img = torch.clamp(model_g_a(source_img), min=0, max=1)
                noised_img = torch.clamp(model_g_b(target_img), min=0, max=1) 
                recovered_img = torch.clamp(model_g_b(fake_img), min=0, max=1)
                recleaned_img = torch.clamp(model_g_a(noised_img), min=0, max=1)
                mse_AB = ((recovered_img - source_img) ** 2).mean().data
                mse_AB_sum += mse_AB
                psnr_AB_sum += -10 * torch.log10(mse_AB)
                mse_BA = ((recleaned_img - target_img) ** 2).mean().data
                mse_BA_sum += mse_BA
                psnr_BA_sum += -10 * torch.log10(mse_BA)
                per_loss_A_sum += g_loss_module.perceptual_loss(source_img, recovered_img)
                per_loss_B_sum += g_loss_module.perceptual_loss(target_img, recleaned_img)

                overall_loss = (per_loss_A_sum + per_loss_B_sum) * 100 + (psnr_AB_sum + psnr_BA_sum) * 0.07
                ...
                if epoch % opt.val_img_interval == 0 and epoch != 0:
                    ...
                    val_image_list = [
                        utils.display_transform()(source_img.data.cpu().squeeze(0)),
                        utils.display_transform()(fake_img.data.cpu().squeeze(0)),
                        utils.display_transform()(recovered_img.data.cpu().squeeze(0)),
                        utils.display_transform()(target_img.data.cpu().squeeze(0)),
                        utils.display_transform()(noised_img.data.cpu().squeeze(0)),
                        utils.display_transform()(recleaned_img.data.cpu().squeeze(0))]
                    n_val_images = len(val_image_list)
                    val_images.extend(val_image_list)

            if opt.saving and len(val_loader) > 0:
                # save validation values
                writer.add_scalar('val/mse_AB', mse_AB_sum / len(val_set), iteration)
                writer.add_scalar('val/mse_BA', mse_BA_sum / len(val_set), iteration)
                writer.add_scalar('val/psnr_AB', psnr_AB_sum / len(val_set), iteration)
                writer.add_scalar('val/psnr_BA', psnr_BA_sum / len(val_set), iteration)
                writer.add_scalar('val/perceptual_error_A', per_loss_A_sum / len(val_set), iteration)
                writer.add_scalar('val/perceptual_error_B', per_loss_B_sum / len(val_set), iteration)
                writer.add_scalar('val/cycle_accuracy', overall_loss / len(val_set), iteration)
