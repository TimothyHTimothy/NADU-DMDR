...
jpeg_dirs = [PATHS[opt.dataset]['jpeg'][opt.resolution]['train'],
               PATHS[opt.dataset]['jpeg'][opt.resolution]['valid']]

for index in range(len(target_dirs)):
    if not os.path.exists(target_dirs[index]):
        os.makedirs(target_dirs[index])
for index in range(len(jpeg_dirs)):
    if not os.path.exists(jpeg_dirs[index]):
        os.makedirs(jpeg_dirs[index])
...
for index, input_dir in enumerate(input_dirs):
    ...
    for file in tqdm(files):
        ...
        jpath = os.path.join(jpeg_dirs[index], os.path.basename(file))
        if opt.noise_std_dev is not None:
            image_2 = utils.gaussian_noise(image, opt.noise_std_dev)
        ...
        if opt.jpg_quality is not None:
            path = os.path.join(target_dirs[index], os.path.basename(file))
            jpeg_path = os.path.splitext(jpath)[0] + '.jpg'
            image.save(jpeg_path, 'JPEG', quality=opt.jpg_quality)
            image = Image.open(jpeg_path)
            os.remove(jpeg_path)
            image.save(jpath, 'PNG')

        image_2.save(path, 'PNG')
