...
parser.add_argument('--device_ids', default=(0,), type=tuple, help='number of workers used')
...
if torch.cuda.is_available():
    ...
    #model_g = nn.DataParallel(model_g, device_ids=opt.device_ids)
    model_d = model_d.cuda()
    #model_d = nn.DataParallel(model_d, device_ids=opt.device_ids)
...
if opt.saving:
    ...
    dir_index = 0
    while os.path.isdir('nova' + save_path + '/' + str(dir_index)):
        dir_index += 1
    summary_path = 'nova' + save_path + '/' + str(dir_index)
...
for epoch in range(start_epoch, opt.num_epochs + 1):
    ...
    if epoch % opt.val_interval == 0 or epoch % opt.val_img_interval == 0:
        ...
        with torch.no_grad():
            ...
            for index, (input_img, _, target_img) in enumerate(val_bar):
                if torch.cuda.is_available():
                ...
                if epoch % opt.val_img_interval == 0:
                    ...
                    val_image_list = [
                        ...
                        utils.display_transform()(input_img.cpu().squeeze(0)),
    ...
    if opt.saving and epoch % opt.save_model_interval == 0 and epoch != 0:
        path = './checkpoints/0526/' + save_path + '/' + opt.artifacts +  '/iteration_' + str(iteration) + '.tar'
        ...
        path = './checkpoints/0526/' + save_path + '/' + opt.artifacts + '/last_iteration.tar'
        torch.save(state_dict, path)
    ...
    for input_img, disc_img in train_bar:
        ...
        if iteration % opt.gen_freq == 0:
            ...
            ## Added, I don't know where the error is...
            g_loss += 0.5 * opt.w_per * g_loss_module.perceptual_loss(g_loss_module.new_filter(fake_img), g_loss_module.new_filter(disc_img))
